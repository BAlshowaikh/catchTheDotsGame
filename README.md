...
- Open the directory in VS Code with `code .`
...
A deployed version of the fully functional game can be found [here](https://dots-game.surge.sh/).


## Instructions

- After opening your game in your browser, open your developer tools, as they will be vital in testing your game's functionality
...
- Inside the section with a class of `level-arena` we have a span tag with two classes - `ball js-ball` (a space between class names applies both classes to the element)
- Inside the header tag, we also have an h2 tag with two classes - `score js-score`
...
- We'll also need to create variables to allow access to our HTML elements and control their behavior
  - Create a variable `ball` and assign its value by grabbing the `.js-ball` class with `querySelector()`
  - Create a variable `scoreDisplay` and assign its value to the element with the `js-score` class
  - Create a variable `levelWinner` and assign its value to the element with the `level-winner` class
...
```
For our game to work we'll need to add logic inside the event listener's callback function. What should happen when the `ball` is `clicked`? Let's break it down.
...
- `console.log` this variable to see it with your developer tools
...
Level 3 also has 3 moving balls but of different sizes! Each ball is therefore worth a different amount of points. The number of points the user should get for each ball is saved in side of an ID on the ball HTML elements. When the user clicks on a ball, increment their score based on how many points that ball is worth until they reach 100.
...
- Next, when a ball is clicked, we'll need to increment the score by each ball's `id` value
- This can be accomplished in several ways
  - <details><summary>HINT 1</summary><p>
  
    Inside your event listener for each ball, `console.log` the `id` of the ball.

    ```js
    console.log(ball.id)
    ```

    How could we use this to increment our score?
    </p>
    </details>
  - <details><summary>HINT 2</summary><p>
  
    Notice that `id` is an *attribute* of each ball element.
...
- Now that we've accessed the increment value of each ball, we'll need to set it to a variable so we'll have access to it in our event listener callback function and make sure that the value is actually an integer value, since this is not guaranteed
...
In this exercise, we used DOM manipulation in multiple scripts and HTML files to access HTML elements as variables, then used event listeners with callback functions to allow for conditional game logic in our Dot game.
...
- `.addEventListener()` is used to handle events for a DOM element. It requires at least 2 arguments:
  - First, the `type` of event being listened for needs to be specified. Commonly used events include: `click`,`DOMContentLoaded`, and `mouseover` but a full list can be found [here](https://developer.mozilla.org/en-US/docs/Web/Events#standard_events).
  - Secondly, something needs to happen once the event being listened for occurs, a *callback* function! The callback function is typically an *anonymous* function `() => {}` or a pre-defined function that determine website behavior after the event occurs.
